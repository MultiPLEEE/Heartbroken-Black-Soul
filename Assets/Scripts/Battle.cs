...
public class Battle : IEventStep
{
    ...
    private void PlayerEventInputOnPlayerConfirm(object sender, EventArgs e)
    {
        switch (_currentMenu)
        {
            ...
            case MenuType.ItemSelect:
                Debug.Log($"Use Item! {_info.inventory.GetInventorySlots()[_currentItemIndex].item.objectName}");
                if (_info.inventory.GetInventorySlots()[_currentItemIndex].item.isTakesTurn)
                {
                    EndPlayerTurn();
                }
                else
    ...
    public void Update()
    {
        if (!IsRunning || _isWaitingForPlayerTurn) return;
    ...
    {
        if (_info.inventory.GetInventorySlots().Count == 0)
        {
